...
let markers = [];
const map_id = get_map_id();

function get_map_id() {
    var url = window.location.href;
    var url_split = url.split("/");
    var map_id = url_split[url_split.length - 1];
    return map_id;
}
...
async function initMap() {
  ...
  document
    .getElementById("pan-to-current-location")
      ...
      if (results[0]) {
        ...
        infoWindow.setContent(
          '<div id="info-window">' +
            ...
            latLng.lng() +
            "</p>" +
            '<button id="add-location" hx-post="{% url maps:get_marker_list%}" hx-target="#markerslist"  hx-swap="outerHTML" hx-swap-oob="true" onclick="createMarker.addLocation()" method="POST" value=' +
            map_id + 
            ">Add Location</button>" +
  ...
  geocoder.geocode({ latLng: latlng }, function (results, status) {
    if (status == google.maps.GeocoderStatus.OK) {
      if (results[1]) {
        ...
        //city data
        location_name =
          city.long_name + ", " + region.long_name + ", " + country.short_name;
